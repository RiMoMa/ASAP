...
UnetDetectionTool::UnetDetectionTool(AnnotationWorkstationExtensionPlugin* plugin, PathologyViewer* viewer)
{
    _annotationPlugin = plugin;
    _viewer = viewer;
}

std::string UnetDetectionTool::name()
{
    return std::string("unetdetection");
}

QAction* UnetDetectionTool::getToolButton()
{
    if (!_button) {
        _button = new QAction("&Detect Glands UNet", this);
        ...
        connect(_button, &QAction::triggered, this, &UnetDetectionTool::runDetection);
    }
    return _button;
}

void UnetDetectionTool::runDetection()
{
    ...
    QString script = QStringLiteral(SCRIPTS_DIR) + "/detect_glands_unet_fov.py";
    ...
    if (!proc.waitForFinished(-1)) {
        qWarning() << "UNet detection script failed:" << proc.errorString();
        return;
    }
